...
v1.2 新增有线充电场景，同时保留原有无线加速：

- **无线模式**继续使用 ARVR（`sconfig=9`）；**有线模式**使用 hp-normal（`sconfig=500`）。两个模式互斥，可分别启用或关闭。
- hp-normal 仅在 Redmi K80 Pro（设备代号 `miro`）、USB/AC 有线供电且屏幕亮起时应用。条件不满足时保持待命，不会强制写入 500；满足条件后由场景守卫监控并维持该场景。
- 拔掉有线充电或熄屏后，如果当前仍是本 App 写入的 500，则恢复默认场景 0。切换模式或正常停止服务时，只清理由本 App 管理的场景，不覆盖其他来源设置的热控场景。
- 前台界面和常驻通知显示所选模式、当前场景及等待条件。系统原有热控与充电策略仍然运行；hp-normal 是依据分析文档 4.5 选出的实测候选，不保证一定提高充电电流，实际效果仍受充电协议、温度和系统策略影响。
- 重装后若显示“读取失败”，请在 KernelSU/Magisk 中确认已允许 Thermal Boost 获取 root 权限。
